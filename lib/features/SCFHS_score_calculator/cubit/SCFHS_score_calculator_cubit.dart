...
class ScfhsScoreCalculatorCubit extends Cubit<SCFHSScoreCalculatorStates> {
  ScfhsScoreCalculatorCubit(this._calculatorRepository) : super( SCFHSScoreCalculatorInitialState());
  ...
  Future<CalculateResultModel?> getCalculateResult() async {
    ...
    final result = await _calculatorRepository.calculateScfhs(realExamController.text, gpaController.text, selectedCvIds);
    result.when(success: (success) {
