...
class MainLayoutRepository {
  ...
  Future<ApiResult<InvoicesResponseModel>> getGifts(int page) async {
    try {
      final response = await _dioFactory.get(
        endPoint: '${EndPoints.gifts}?page=$page',
      );
      if (response!.statusCode == 200) {
        final InvoicesResponseModel model = InvoicesResponseModel.fromJson(
          response.data,
        );
        return ApiResult.success(model);
      } else {
