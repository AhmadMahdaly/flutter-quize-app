  ...
  Widget build(BuildContext context) {
    return BlocConsumer<SubscriptionCubit, SubscriptionStates>(
      ...
      builder: (context, state) {
        ...
        if (state is GetStoreProductsLoadingState || state is GetPackagesLoadingState) {
          return  Scaffold( appBar: CustomAppBar(title: 'subscription'.tr(context)),body: const Center(child: CircularProgressIndicator()));
        }
        if (state is GetStoreProductsFailedState) {
          return Scaffold( appBar: CustomAppBar(title: 'subscription'.tr(context)),body: Center(child: Text('Error: ${state.message}')));
        }
        ...
        final allBackendPackages = (cubit.packagesModel?.data ?? []) + (cubit.extraPackagesModel?.data ?? []);
            ...
            child: Column(
              ...
              children: [
                ...
                ...List.generate(cubit.storeProducts.length, (index) {
                  final product = cubit.storeProducts[index]; // المنتج من متجر Apple

                  // البحث عن الباقة المطابقة من السيرفر لجلب الميزات والبيانات الوصفية
                  final backendPackage = allBackendPackages.firstWhere(
                        (pkg) => pkg.appleProductId == product.id, // افترضنا اسم الحقل appleProductId
                  ...
                  return Padding(
                    ...
                    child: GestureDetector(
                      ...
                      onTap: () {
                        if (state is! PurchaseLoadingState && state is! PurchaseVerificationLoadingState) {
